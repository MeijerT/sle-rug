...
AForm testflatten() {
  return flatten(cst2ast(parse(#start[Form],|project://QL/examples/test.myql|)));
}
...
start[Form] testrename() {
  f = parse(#start[Form], |project://QL/examples/cyclic.myql|);
  AForm af = cst2ast(f);
  return rename(f, |project://QL/examples/cyclic.myql|(154,12,<8,6>,<8,18>), "test", resolve(af));
}
